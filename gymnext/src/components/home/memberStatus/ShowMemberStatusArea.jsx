...
const codePack = member?.data[0]?.code;

//alert("Im in showMemberStatusArea line 27 - codePack: "+codePack)
      ...
      <div className="window rounded-3">
       <div className="row" style={{display:'flex', flexDirection:'row', justifyContent:'center', flexWrap:'wrap'}}>
        ...
        <div className="col-md-5" style={{margin:'2em', maxWidth:'85%', minHeight:'100%'}}>            
          <img className="img-fluid rounded-3" src={`${BackendURL}`+pack?.data[0]?.image}  style={{margin:'1em',  }} height={3000} width={3000} alt="Gym_logo"/>
       </div>
       ...
       {/**<!---- Username and Email section -------------------->   **/}                   
        ...
        <h4>Pack Name: {member?.data[0]?.nameplan}</h4>
        <h4>Time Days: {member?.data[0]?.timedays}</h4>
        <h4>Price: {member?.data[0]?.cost} $</h4>
     ...
     <div className="mb-3">
      <label for="image" className="control-label" style={{color:'aliceblue', fontWeight:'bold'}}>Member Picture:
         ...
         <div className="file-custom"  >
            ...
            <Image  src={`${BackendURL}`+member?.data[0]?.imageUser} style={{margin:'1em', height:'10em', width:'8em'}} height={1000} width={2000} alt="User Image"/>
         </div>
