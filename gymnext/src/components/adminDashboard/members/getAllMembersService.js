...
export async function GetAllMembersService (setMembers, SetDaysLeft_mathFloor)  {
...
const MillisecondsPerDay = (1000 * 60 * 60 * 24);
    ...
    try {
      ...
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}`+'api/member/listAll')
      // if got data successfully --> change local Product.state with setCategories(res.data.data)
      // new values
      //alert('Estoy en getallmembersSErvice - line 11 - res.data.message:'+res.data.message); // line only to test res data
        //alert('Estoy en getAllMembersService - line 12 - res.data.total:'+res?.data?.total);   // line only to test res data
